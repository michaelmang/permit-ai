...
- **Build (static export):** `npm run build` — `next.config.js` sets `output: 'export'`, so a full static site is emitted to `out/`. `next build` also runs ESLint/type checks; there is no separate `lint` or `test` script.
